...
def to_pc_fmt(
    ...
    if output_filepath.exists():
        ...
        typer.echo(f"Output file {output_filepath} already exists. Overwriting.")
    ...
    # Add delta_t to each message
    messages.sort(key=lambda x: x["timestamp"])
    messages[0]["delta_t"] = 0.0  # First message has no delay
    for i in range(1, len(messages)):
        messages[i]["delta_t"] = messages[i]["timestamp"] - messages[i - 1]["timestamp"]

    # Write to output file in persona-chat format
